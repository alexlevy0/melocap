-- Migration: Secure save_stakes RPC with membership check
-- Path: supabase/migrations/009_secure_save_stakes.sql
...
BEGIN
  ...
  -- 2. Verify theme status (must be 'open')
  SELECT t.status INTO v_theme_status
  FROM public.pods p
  ...
  -- 3. Verify Pod Membership (SECURITY FIX)
  SELECT EXISTS (
  ...
  -- Insert new stakes
  INSERT INTO public.stakes (user_id, submission_id, amount)
  SELECT v_user_id, (s->>'submission_id')::UUID, (s->>'amount')::INTEGER
